...
- **状态：** `technical_review`
- **演示边界：** 10 条官方页面核验机会（UArizona Bear Down×6、AdventureX、CHI 2027×2、ICRA 2027）+ 6 条明确虚构机会；学生 Profile、匹配结果、提供方回应、权益与申请结果均为 `demo_fixture`
...
|---|---|---|---|
| F009-01 官方机会采集 | ✅ | 16 条机会均声明 `campus` / `external` 范围、来源 URL、版本、抓取/核对时间、日期、成本与纠错渠道；其中 10 条使用官方页面核验快照 | 未连接实时采集器或私人邮箱；动态字段仍需提交前复核 |
| F009-02 透明机会卡 | ✅ | Market Board 先分 Bear Down League / Open League，再显示资格、收益、义务、成本与风险；学术会议不与校内活动混排 | 等待真人文案/密度评审 |
| F009-03 透明机会包 | ✅ | 3 个主题集合、16 项全部公开；集合可跨范围，但发现阵容必须分区；无概率、稀有度、付费解锁或倒计时 | 无真实运营集合 |
| F009-04 资格规则 | ✅ | 35 条结构化规则均保留来源链接；`lte` 使 AdventureX 2–4 人上下限可分别计算 | 未接真实规则版本监控 |
| F009-05 四态解释 | ✅ | 同一研究机会可同时显示 met / possibly_met / not_met / unknown；无综合分 | 真实规则准确性未验证 |
| F009-06 本人控制 Profile | ✅ | 15 个私密可选字段；当前参赛 Fixture 默认选择 10 个，UArizona first-year 状态保持未选且须本人确认；敏感代理字段进入禁止清单 | 未接真实身份或凭证 |
| F009-07 选择性匹配 | ✅ | 本人逐项选择字段，14 条有效机会显示理由、冲突与未知；范围先分区，区内按资格四态→截止时间确定性排序 | 未做真实公平样本评估 |
...
- [x] 16 条透明机会：校内 10 条（有效 8 条）、校外 6 条（有效 6 条），默认分区不混排
- [x] 10 条官方页面核验机会 + 6 条明确 Fixture；3 个透明主题集合
- [x] 35 条来源可追溯资格规则（含数值上下限）
- [x] 四态同屏与无综合分
...
- [x] 离线只读、简化视图、减少动效
- [x] 浏览器逐步实机验收并重置为干净状态
- [x] 16 项 Web 引擎测试、4 项领域测试、3 项 API 流程测试
- [x] AdventureX 终极指南核验快照转为团队 / 时段 / 提交包四项准备检查；动态 Portal 与视觉许可保持人工 Gate
- [x] Bear Down Camp、Wildcat Student Employment & Research Fair、Bear Down Week、Bear Down Fall Festival、Clubs & Organizations 与 Bear Down Music Festival Replay 均保留 UArizona 官方来源、Arizona Time、成本未知项和人工复核边界
