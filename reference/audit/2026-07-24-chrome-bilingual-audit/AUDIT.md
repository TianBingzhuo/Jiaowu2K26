...
角色切换、语言切换、双语解说与代表性主路径；当前 18 个测试文件合计
142 条通过，其中双语 Catalog、角色说明、伪本地化、ICU 俄语复数规则与
非阻塞英文状态提示均有回归守卫。实体手柄、Firefox、Safari 与
真实跨语言参与者仍是独立证据。
...
F-001 学生答题页曾出现英文 `NOW PLAYING` 点击层覆盖“提交答案并查看依据”的问题。
浏览器几何命中确认点击实际落在双语 HUD，而不是主 CTA。现已把该元素改为
`role="status"` 的非交互提示并保留独立 `EN Guide` 按钮；Edge/Chromium 重新走完
F-001 7/7 主路径。该修复说明“无页面级溢出”不等于“没有固定浮层遮挡”，后续
主路径验收必须继续包含真实点击和 `elementFromPoint` 命中检查。
