# 大学2K26 / University2K26 · AI 与队友任务路由器
...
新队友或新 AI 第一次接手时，AI 不得只回复“已经了解”“可以开始”。只有以下三项都由**队友本人明确确认**后，才可写“入场确认完成”：

1. **协作角色与任务：** 给出建议角色、当前唯一任务、为什么现在做、输入、交付物、Done 和禁止事项；角色来自本人说明或待本人确认，不能秘密推断。
2. **任务相关技术熟悉度：** 只列本任务实际用到的技术，让本人逐项标记 `熟悉 / 可在辅助下完成 / 不熟悉`；AI 不得替本人宣称熟悉。
3. **任务内容与目标：** 用自己的话复述目标、非目标、成功证据，并把建议、风险和待确认问题单列，不能实施后才暴露方向分歧。

技术存在不熟悉项时先暂停实施，提供三种可审选择：`保留并结对`、`改派任务`、`替换该层实现`。由队友和产品总集成人员决定；AI 不得擅自更换共享合同、全局架构或已经验收的技术。团队熟悉度高于技术新奇度，但更换实现仍须通过当前 Gate。
...
角色与任务：<建议 role> / <task_id> / <为什么现在做>
你需要读取：<required_reads>
交付与验收：<outputs> / <acceptance>
不要做：<forbidden>
任务相关技术栈：
- <technology>: <等待本人填写：熟悉 / 可在辅助下完成 / 不熟悉>
目标复述：<目标、非目标、成功证据>
建议与风险：<建议、风险、待确认问题；没有也明确写“暂无”>
请本人确认或调整以上三项；确认不等于认领任务。
```

收到本人确认后，AI 可回复以下状态，但仍不得自动认领：

```text
入场确认完成
角色：<confirmed_role>
候选任务：<task_id>
技术熟悉度与处置：<confirmed_result>
目标与边界：<confirmed_summary>
下一步：回复“我认领 <task_id>，角色是 <role/member_id>”
```
...
- 维护者工作站的稳定项目入口是 `D:\10451\Desktop\黑客松`，它位于批准的 `D:\10451` junction 下；普通 GitHub 克隆使用各自的克隆根目录，不要求存在该盘符。
- 一次任务只使用当前克隆根目录。维护者工作站不得把该入口与等效 C 路径或 `D:\DiskC` 混合去重、索引或同步。
- 维护者工作站不写 D 盘其他非 junction 目录；其他机器遵守自己的工作区边界。
