...
- **状态：** `technical_review`
- **演示边界：** 4 条官方页面核验机会（AdventureX、CHI 2027×2、ICRA 2027）+ 6 条明确虚构机会；学生 Profile、匹配结果、提供方回应、权益与申请结果均为 `demo_fixture`
...
|---|---|---|---|
| F009-01 官方机会采集 | ✅ | 10 条机会均声明 `campus` / `external` 范围、来源 URL、版本、抓取/核对时间、截止、成本与纠错渠道；其中 4 条使用官方页面核验快照 | 未连接实时采集器；动态字段仍需提交前复核 |
| F009-02 透明机会卡 | ✅ | Market Board 先分 Campus League / Open League，再显示资格、收益、义务、成本与风险；学术会议不与校内活动混排 | 等待真人文案/密度评审 |
| F009-03 透明机会包 | ✅ | 2 个主题集合、10 项全部公开；集合可跨范围，但发现阵容必须分区；无概率、稀有度、付费解锁或倒计时 | 无真实运营集合 |
| F009-04 资格规则 | ✅ | 29 条结构化规则均保留来源链接；`lte` 使 AdventureX 2–4 人上下限可分别计算 | 未接真实规则版本监控 |
| F009-05 四态解释 | ✅ | 同一研究机会可同时显示 met / possibly_met / not_met / unknown；无综合分 | 真实规则准确性未验证 |
| F009-06 本人控制 Profile | ✅ | 14 个私密可选字段；当前参赛 Fixture 默认选择 7 个，其中团队、现场时段与提交包均可单独撤回；敏感代理字段进入禁止清单 | 未接真实身份或凭证 |
| F009-07 选择性匹配 | ✅ | 本人逐项选择字段，9 条有效机会显示理由、冲突与未知；范围先分区，区内按资格四态→截止时间确定性排序 | 未做真实公平样本评估 |
...
- [x] 10 条透明机会：校内 4 条（有效 3 条）、校外 6 条（有效 6 条），默认分区不混排
- [x] 4 条官方页面核验机会 + 6 条明确 Fixture；2 个透明主题集合
- [x] 29 条来源可追溯资格规则（含数值上下限）
- [x] 四态同屏与无综合分
