...
- `/api/v1/health` 负责启动握手；F-001 另会只读获取 generated object 与 Replay，
  通过 v1 嵌套合同校验后显示 API 回执。当前 Studio 的编辑、审核、发布与学生互动
  仍是可重置的本地 Fixture，不会暗中写入后端。
...
API 与 Web 默认只允许回环访问。当前 `actor_id` 只是公开 Fixture 的审计标签；
没有服务端身份和授权。不要为了让其他设备访问而直接改成 `0.0.0.0`，除非先完成
受控反向代理、认证授权、TLS、限流和请求体上限。
