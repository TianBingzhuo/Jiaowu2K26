...
- `reference/audit/2026-07-25-copy-voice-audit/AUDIT.md`：以 Qoder 实机审计为基线，完成五组同视口前后截图、九模块深层可见文案扫描和关键交互复测；新增 16 个主界面 + 10 个演示数据文件的产品声音守门测试。
- `engineering/LOCALIZATION-AND-TONE.md` 增补“会接梗的同桌，关键时刻很稳的队友”产品声音、五种说话层级、梗配额、内部术语映射和反生成式套话约束。
...
- 学生首页、九个 F 模块、五角色工作台、AI 本地规则回复与 Rust API 演示边界完成一轮人类化改写；`Fixture`、`fallback`、`authoritative=false` 等工程字段不再直接出现在正常用户界面，内部契约和来源 ID 保持不变。
- SmartCourse 答题未选择提示会在选中答案后立即解除；Replay 将“完成度”和“答对数”拆开，未命中时引导回看来源而不把一次结果写成人格标签。
